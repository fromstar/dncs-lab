...
For the router-1/router-2 connection I used a subnet with /30 subnet mask(it allows to have only two 2 assignable addesses plus one network address and one broadcast address):

- link router-1<->router-2: 192.168.1.0 (addresses from 192.168.1.1 to 192.168.1.4)
...
So I let go the new machine's default route through their subnet's gateway.
The default routes are so set in this way:
- host-a: via 172.16.0.1
- host-b: via 172.16.2.1
- router-2: via 192.168.1.1
- host-c: via 172.16.4.1

In router-1, since it is adjacent to router-2 and that belongs to the vlan, it was enough for me to just add a specific route for the host-c subnet via router-2 to address 192.168.1.2
However the operations made until here are not enough to allow packet forwarding, in fact I had to enable also the ipv4 forwarding in the router-1 and in the router-2 to make everything work.
